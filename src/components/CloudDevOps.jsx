...
export default function CloudDevOps({ onSelectService }) {
  const { cloudDevOps } = companyData;

  const cloudIconMap = {
    'cloud-consulting': Cloud,
    'cloud-migration': ArrowUpRight,
    ...
    'devops': GitBranch,
    'cicd': Terminal,
    'infra-automation': Layers,
    'cloud-monitoring': Activity,
  };

  return (
    <section id="cloud-devops" className="py-20 lg:py-28 bg-[#f8fafc] border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mb-14">
          <div className="inline-flex items-center space-x-2 text-xs font-semibold uppercase tracking-wider text-[#0284c7] mb-3">
            ...
            <span>SECTION 05 &bull; {cloudDevOps.category.toUpperCase()}</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 leading-tight">
            {cloudDevOps.title}
          </h2>
          <p className="mt-3 text-base text-slate-600 leading-relaxed">
            {cloudDevOps.description}
          </p>
        </div>

        {/* Visual & Services Architecture */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
          
          {/* Server Infrastructure Photo (Left 5 Cols) */}
          <div className="lg:col-span-5">
            <div className="sticky top-28 space-y-4">
              <div className="overflow-hidden rounded-xl bg-slate-200 border border-slate-300 shadow-sm">
                <img
                  src={corporatePhotography.cloudDevOps.url}
                  alt={corporatePhotography.cloudDevOps.alt}
              ...
              <div className="p-4 rounded-lg bg-white border border-slate-200 text-xs text-slate-600 space-y-2">
                <div className="flex items-center justify-between font-semibold text-slate-800">
                  <span>SRE & High-Availability Baseline</span>
                  <span className="text-[11px] font-mono text-emerald-600 font-bold">99.99% Target SLA</span>
                </div>
                <p className="text-slate-500 leading-relaxed">
                  Infrastructure is defined strictly as version-controlled code. Multi-region redundancy, automated failover drills, and continuous blue/green deploy gates mitigate outage risk.
                </p>
                <div className="pt-2 flex flex-wrap gap-2 text-[11px] font-mono text-slate-600">
                  <span className="px-2 py-0.5 bg-slate-100 rounded">Terraform</span>
                  <span className="px-2 py-0.5 bg-slate-100 rounded">Kubernetes</span>
                  <span className="px-2 py-0.5 bg-slate-100 rounded">AWS EKS</span>
                  <span className="px-2 py-0.5 bg-slate-100 rounded">Prometheus</span>
                </div>
              </div>
            </div>
          </div>

          {/* 10 Cloud Services (Right 7 Cols) */}
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {cloudDevOps.services.map((service) => {
              const Icon = cloudIconMap[service.id] || Cloud;
              return (
                <div
                  key={service.id}
                  onClick={() => onSelectService && onSelectService({ ...service, category: cloudDevOps.title })}
